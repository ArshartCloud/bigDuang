 ...
 */
public class ShowtimeAdapter extends BaseAdapter implements View.OnClickListener {

    private Context context;
    private List<Showtime> screenList;
...
//        Button mybu;
    ...
    public ShowtimeAdapter (Context c, List<Showtime> list) {
        if (null != list) {
            screenList= list;
        } else {
            screenList= new ArrayList<Showtime>();
        }
        this.context = c;
    ...
    public void addItem(Showtime item) {
        if (item != null) {
            screenList.add(item);
    ...
    public void addItem(List<Showtime> list) {
...
//            holder.mybu = (Button) convertView.findViewById(R.id.button1);
        ...
        Showtime screen = screenList.get(position); // 获取当前项数据
        if (null != screen) {
//            holder.time.setText(screen.getDate().toString());
            holder.time.setText(screen.getDate().toString());
//            holder.description.setText(screen.getChooseItemEC());
            holder.price.setText("Price: $" + Double.toString(screen.getPrice()));
//            holder.mybu.setTag(screen.getId());
        }
        return convertView;
    }
